...
def replan(env, curr_fluents, init, goals, init_plan):
	flag, idx = env.find_logical_states(curr_fluents, goals=goals)
	if flag is True:
		# At a seen state, find which one in the goals
		return init_plan[int(idx*2):]
	else:
		"""solve multi-goal TAMP problem"""
		multi_goal = ("or", *goals)
...
def add_disturbances(env, blocks_info, level=0):
	...
	if level == 2:
		...
		new_object_pos = blocks_info["cube_c"]["pose"]
		env.set_point(cube_c_id, new_object_pos)
		return None
...
def L2_main():
	...
	for itr in range(MAX_ITER):
		...
		disturb_init = None
		if itr == 1:
			disturb_init = add_disturbances(env, blocks_info, level=2)
		# generate current logical states
		statics, curr_fluents = env.get_logical_state(robots_info=robots_info,
										  tables_info=tables_info,
										  blocks_info=blocks_info)
...
if __name__ == '__main__':
	...
	replan_cost_t_list = []
	for _ in range(10):
		task_cost_t, replan_cost_t = L4_main()
	...
	# 				   "reactive_tamp", "block_stacking")
	# if not os.path.exists(folder_path):
	# 	os.makedirs(folder_path)
	# np.savetxt(absjoin(folder_path, "L4_task_cost_t_list.txt"), 
	# 		np.asarray(task_cost_t_list))
	# np.savetxt(absjoin(folder_path, "L4_replan_cost_t_list.txt"), 
